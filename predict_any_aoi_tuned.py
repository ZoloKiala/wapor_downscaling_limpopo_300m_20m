...
# -*- coding: utf-8 -*-
"""
Apply a tuned 20 m RF downscaling model to ANY AOI for ONE dekad date.

Key features
------------
- Downloads a 20 m predictor stack (S2, NDVI, S1 dB+GLCM, CHIRPS & lag, DEM, Slope, LC)
  for the given AOI/date via Earth Engine + geedim.
- Loads your tuned RF model + selected feature list, predicts 20 m ETa.
- Clips to the WaPOR ETa300m footprint (always included for viz) and writes a
  "Predicted vs WaPOR ETa" PNG alongside quick metrics.
- Optional: only build/download the predictors actually needed by the tuned model
  (use --features-aware) to reduce time & IO.
- Optional: --overwrite to force a re-download.

Auth (non-interactive options; do NOT put JSON on CLI if you want it hidden):
...
CLI example
-----------
python predict_any_aoi_tuned.py \
  --aoi "bbox:16.2,-29.1,17.7,-28.4" \
  --date 2020-01-01 \
  --model wapor_20m_local/outputs_rf20_tuned/rf_model_tuned.joblib \
  --features wapor_20m_local/outputs_rf20_tuned/features_tuned.json \
...
# Headless plotting backend (servers/WSL)
...
MIN_VALID_BYTES = 64 * 1024  # sanity check for reuse
...
# ------------------------------------------------------------
# Feature-aware stacking helpers
# ------------------------------------------------------------
def _which_sources(features):
    """Figure out which EE sources/bands we actually need (besides ETa300m, always kept for viz)."""
    fset = set(features or [])
    need = {
        "S2": bool({"B4","B8","B11","NDVI"} & fset),
        "S1": any(f.startswith("S1_") for f in fset),
        "CHIRPS": any(f.startswith("CHIRPS") for f in fset),
        "DEM": bool({"DEM","Slope"} & fset),
        "LC": ("LC" in fset) or any(f.startswith("LC_") for f in fset),
        "ETa300m": True,  # always include for Pred vs WaPOR viz
    }
    return need

def _build_stack_for_date(date_iso: str, region_fc: ee.FeatureCollection, features=None):
    """Build a minimal EE image containing only the requested predictors + ETa300m for visualization."""
    need = _which_sources(features or [])
    ...
    if need["S2"]:
        ...
        keep = []
        if "B4" in (features or []):  keep.append("B4")
        if "B8" in (features or []):  keep.append("B8")
        if "B11" in (features or []): keep.append("B11")
        if keep:
            parts.append(s2.select(keep))
        if "NDVI" in (features or []):
            parts.append(s2.normalizedDifference(["B8","B4"]).rename("NDVI"))

    if need["S1"]:
        parts.append(_s1_dekad_safe(start, end, region_fc))

    if need["CHIRPS"]:
        parts.append(_chirps_pair(start, end))

    if need["DEM"]:
        dem   = ee.Image(SRTM_ID).rename("DEM")
        slope = ee.Terrain.slope(dem).rename("Slope")
        keep = []
        if "DEM" in (features or []):   keep.append("DEM")
        if "Slope" in (features or []): keep.append("Slope")
        if keep:
            parts.append(ee.Image.cat([dem, slope]).select(keep))
    ...
    if need["ETa300m"]:
        parts.append(_l1_dekad_safe(start, end, region_fc))

    if not parts:
        raise RuntimeError("No EE sources required by the selected features.")

    stack = ee.Image.cat(parts)

    # Final select: only the bands we’ll feed the model (ignore DAY_* which is local).
    # NB: If features contain LC_* one-hots, we still need base "LC" to compute them locally.
    final_bands = [f for f in (features or []) if f not in ("DAY_SIN","DAY_COS") and not f.startswith("LC_")]
    if any(f.startswith("LC_") for f in (features or [])) and "LC" not in final_bands:
        final_bands.append("LC")
    # Always keep ETa300m for viz (if present)
    if "ETa300m" not in final_bands:
        final_bands.append("ETa300m")

    # Some features might be purely seasonal; if final_bands is empty, keep ETa300m to ensure a valid image.
    if not final_bands:
        final_bands = ["ETa300m"]

    stack = stack.select(final_bands)
    return stack

# ------------------------------------------------------------
# Geedim download
# ------------------------------------------------------------
def ensure_stack_download(date: str, aoi: str, out_root: str,
                          overwrite=False, sa_json: str | None = None,
                          features=None) -> tuple[str,str]:
    ...
    if os.path.exists(stack_fp) and not overwrite and os.path.getsize(stack_fp) > MIN_VALID_BYTES:
        print("[STACK] Reusing:", stack_fp)
        return stack_fp, aoi_name

    stack = _build_stack_for_date(date, fc, features=features)
...
def predict_stack_blockwise(fp_in, rf, features, fp_out, date_iso,
            ...
            for y0 in range(0, H, chunk):
                for x0 in range(0, W, chunk):
                    ...
                    for f in features:
                        ...
                        elif f not in ("DAY_SIN","DAY_COS") and f != "ETa300m":
                            need.add(f)
                    raw = {}
                    ok = True
                    for f in need:
                    ...
                    for f in features:
                        if f == "DAY_SIN":
                            ...
                            # never a predictor for tuned flow; ignore if present
                    ...
                    # Remove placeholders (ETa300m) if any
                    cols = [c for c in cols if c is not None]
                    block = np.stack(cols, axis=0)
                    X = np.moveaxis(block, 0, -1).reshape(-1, len(cols))
...
def _read_eta300_from_stack(ds):
    ...
    # Fall back to band named exactly "ETa300m" if descriptions missing
    if "ETa300m" in bm:
        return ds.read(bm["ETa300m"]+1).astype(np.float32)
    # Soft fallback: try band #5 if present (as in many full stacks); may be absent in feature-aware mode
    try:
        if ds.count >= 1:
...
def clip_to_eta300_mask_pair(pred_tif, stack_tif, out_pred_tif, out_eta_tif, nodata=NODATA_OUT):
    with rasterio.open(stack_tif) as ds, rasterio.open(pred_tif) as dp:
        ...
        if eta is None:
            raise KeyError("ETa300m band not found in downloaded stack (needed for viz).")
...
def _load_model_and_features(model_path, feats_path=None):
    ...
    if (features is None) and feats_path and os.path.exists(feats_path):
        with open(feats_path, "r") as f:
            meta = json.load(f)
            # support {"features":[...]} or {"features":{"selected":[...]}}
            if isinstance(meta.get("features", None), dict) and "selected" in meta["features"]:
                features = meta["features"]["selected"]
            else:
                features = meta.get("features", None)
            # support {"selected":[...]} top-level too
            if features is None and "selected" in meta:
                features = meta["selected"]
    if features is None:
        maybe = os.path.join(os.path.dirname(model_path), "features_tuned.json")
        if os.path.exists(maybe):
            with open(maybe, "r") as f:
                meta = json.load(f)
                features = meta.get("features", meta.get("selected", None))
    if features is None:
        raise RuntimeError("Could not recover tuned feature list; provide --features path.")
    return rf, features
...
def run_any_aoi(aoi: str, date: str, model_path: str, feats_path: str | None,
                ...
                overwrite: bool = False, show_inline: bool = False, features_aware: bool = False):
    """
    ...
    # When features-aware, only build/download the predictors the tuned model really needs (+ETa300m for viz)
    dl_features = features if features_aware else None
    stack_fp, aoi_name = ensure_stack_download(date, aoi, out_root=out_root,
                                               overwrite=overwrite, sa_json=sa_json,
...
def main():
    ...
    ap = argparse.ArgumentParser(description="Apply tuned RF model to ANY AOI for ONE dekad date.")
    ...
    ap.add_argument("--model", required=True, help="Path to rf_model_tuned.joblib (or bundle with {'rf','features'})")
    ap.add_argument("--features", default=None, help="Path to features_tuned.json (optional if embedded)")
    ...
    ap.add_argument("--features-aware", action="store_true",
                    help="Only download/build predictors actually used by the tuned feature list (+ETa300m)")
    ap.add_argument("--overwrite", action="store_true", help="Force re-download even if the stack exists")
    args = ap.parse_args()

    res = run_any_aoi(
        aoi=args.aoi, date=args.date,
        model_path=args.model, feats_path=args.features,
        out_root=args.out_root, sa_json=args.sa_json,
        overwrite=args.overwrite, show_inline=False,
        features_aware=args.features_aware
    )
