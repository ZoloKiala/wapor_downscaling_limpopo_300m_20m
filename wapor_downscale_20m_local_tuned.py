...
Follows the original script's approach:
  1) Mutual Information filter → shortlist top-K (default 10)
  2) Greedy forward selection with GroupKFold by file (R²), max 12 steps
  3) RandomizedSearchCV on the selected subset (CV=5)
Land cover (LC) is kept as a numeric feature (no one-hot) in this tuned flow.

Usage examples:
  # Fine-tune with defaults (K=10, max steps=12), export if needed
  python wapor_downscale_20m_local_tuned.py --tune

  # Skip geedim exports (you already have stacks), increase K, cap steps
  python wapor_downscale_20m_local_tuned.py --tune --skip-export --filter-top-k 12 --forward-max-steps 10
"""
...
# ====== base implementation (exports, discovery, IO, baseline RF) ======
# -*- coding: utf-8 -*-
"""
...
TRAIN_START = "2018-01-01"; TRAIN_END = "2018-02-01"   # [start, end)
TEST_START  = "2019-01-01"; TEST_END  = "2019-02-11"   # inclusive

# ------------------ SAMPLING / RF CONFIG ------------------
PER_FILE_SAMPLES  = 3000
CHUNK_PRED        = 1024
RANDOM_STATE      = 7
NODATA_OUT        = -9999.0
...
# ====== Numeric-LC sampling & matrices (match the original script) ======

def sample_from_stack_numericLC(path, features, n=1000):
    """Sample columns for the given feature list from a stack; LC stays numeric."""
    with rasterio.open(path) as ds:
        bm = band_map(ds)
        label = read_label_band(ds, bm)
        nodata = ds.nodata
        ...
        # load needed arrays; skip file if a required band is missing
        raw = {}
        for name in needed:
            if name not in bm:
                return None, None
    ...
    # build validity mask across selected features
    for f in features:
        if f in ("DAY_SIN","DAY_COS"):
            continue
        a = raw.get(f)
...
def build_matrix(files, features, per_file_samples=800):
    """Stack samples from files into a matrix X, y with group index per file."""
    Xs, ys, gs = [], [], []
...
def mi_filter_topk(train_files, base_pool, per_file_samples=800, top_k=10, random_state=RANDOM_STATE):
    """Mutual information filter to shortlist features quickly (LC numeric)."""
...
# ====== Greedy Forward Selection (GroupKFold by file, R²) ======
...
def forward_select(train_files, pool, per_file_samples=800, cv_splits=3,
    ...
    """Greedy forward selection with a small RF and GroupKFold by file.
       Caps splits to available groups; falls back to 80/20 if only 1 group."""
    current = []
    best_score = -np.inf
    steps = 0
...
def tune_rf(X, y, random_state=RANDOM_STATE, n_iter=30, cv_splits=5):
    rf = RandomForestRegressor(**RF_PARAMS)  # same baseline keys as base script
    param_dist = dict(
...
def main_cli():
    ...
    # Filter & forward params (defaults mirror the original script)
    ...
    # Date overrides
    ap.add_argument("--train-start", type=str, default=TRAIN_START)
    ap.add_argument("--train-end", type=str, default=TRAIN_END)
    ap.add_argument("--test-start", type=str, default=TEST_START)
    ap.add_argument("--test-end", type=str, default=TEST_END)
    args = ap.parse_args()
    ...
    # 2) Build base pool from discovered predictors (no one-hot LC), keep seasonal
    base_feats, lc_vals = discover_predictor_names(
        train_files, do_one_hot=False, use_day=USE_DAY_OF_YEAR,
        max_scan=MAX_SCAN_FILES, pct_required=PCT_REQUIRED
    )
    # ensure seasonal present
    if "DAY_SIN" not in base_feats and USE_DAY_OF_YEAR: base_feats += ["DAY_SIN"]
    if "DAY_COS" not in base_feats and USE_DAY_OF_YEAR: base_feats += ["DAY_COS"]

    # 3) MI filter
    shortlist, mi_scores = mi_filter_topk(
        train_files, base_feats, per_file_samples=args.filter_samples, top_k=args.filter_top_k
    )
    pd.DataFrame([{"feature": k, "mi": v} for k, v in mi_scores.items()]).to_csv(
        os.path.join(OUT_DIR, "mi_scores.csv"), index=False
    )

    # 4) Greedy forward selection on shortlist
    selected, best_r2 = forward_select(
        train_files, shortlist, per_file_samples=args.forward_samples,
        cv_splits=args.forward_cv, min_improve=args.forward_min_improve,
        max_steps=args.forward_max_steps
    )
    with open(os.path.join(OUT_DIR, "forward_selected.json"), "w") as f:
        json.dump({"selected": selected, "best_r2_cv": best_r2}, f, indent=2)

    # 5) Training set matrix for final stage
    ...
    # Save model bundle
    dump({"rf": rf, "features": selected}, os.path.join(OUT_DIR, "rf_model_tuned.joblib"))
    with open(os.path.join(OUT_DIR, "features_tuned.json"), "w") as f:
        json.dump({"features": selected}, f, indent=2)

    # 7) Predict & evaluate on test window
    Path(VIS_DIR).mkdir(parents=True, exist_ok=True)
    Path(PRED_DIR).mkdir(parents=True, exist_ok=True)
    records = []
    for fp in test_files:
    ...
    pd.DataFrame(records).to_csv(os.path.join(OUT_DIR, "metrics_tuned.csv"), index=False)
