# -*- coding: utf-8 -*-
# plot_metrics_20m.py — Local Machine Edition
# ------------------------------------------------------------
# Plots time-series metrics and a global Pred vs Native scatter
# for RF downscaling outputs written by outputs_rf20_auto/.
#
# Usage:
#   python plot_metrics_20m.py
#   # or edit CSV_PATH below to your metrics.csv
# ------------------------------------------------------------
...
# ---- CONFIG ----
# Default locations following main script
DATA_DIR = "wapor_20m_local"
OUT_DIR  = os.path.join(DATA_DIR, "outputs_rf20_auto")
...
def plot_time_series(csv_path=CSV_PATH, out_png=None, title="Prediction Accuracy over Time"):
    ...
    plt.tight_layout()
    if out_png is None:
        out_png = os.path.join(OUT_DIR, "accuracy_timeseries.png")
...
def scatter_pred_vs_true(csv_path=CSV_PATH, sample_per_file=3000, out_png=None, title="Pred vs. Native (sampled)"):
    df = pd.read_csv(csv_path)
    xs, ys = [], []
    ...
    if out_png is None:
        out_png = os.path.join(OUT_DIR, "scatter_pred_vs_native.png")
    plt.tight_layout()
